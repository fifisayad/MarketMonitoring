...
def create_exchange_worker(
    exchange: Exchange, market: Market, monitoring_repo: MonitoringSHMRepository
) -> BaseExchangeWorker:
    if exchange == Exchange.HYPERLIQUID:
        return HyperliquidExchangeWorker(market=market, monitoring_repo=monitoring_repo)
    elif exchange == Exchange.BINANCE:
        return BinanceExchangeWorker(market=market, monitoring_repo=monitoring_repo)
    else:
